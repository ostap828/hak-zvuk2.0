...
class History(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    # Исправлено: ForeignKey ссылается на users_id из таблицы users
    telegram_id_users = Column(BigInteger, ForeignKey('users.users_id', ondelete='CASCADE'), nullable=False)
...
async def create_tables():
    ...
    logger.info("Таблицы удалены и созданы заново")
...
async def create_history(
    ...
    try:
        new_history = History(
            telegram_id_users=telegram_id_users,
...
async def get_user_history(session, telegram_id_users: int) -> list[History]:
    """Получение всей истории пользователя"""
    query = select(History).where(History.telegram_id_users == telegram_id_users)
    result = await session.execute(query)
